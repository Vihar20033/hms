...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, DialogModule, ButtonModule, InputTextModule, InputTextareaModule],
...
export class RecordTestResultsComponent implements OnInit {
  ...
  initForm(): void {
    this.resultsForm = this.fb.group({
      findings: ['', [Validators.required, Validators.minLength(5), Validators.maxLength(5000)]],
  ...
  onSubmit(): void {
    ...
    this.labReportService.createReport(this.labTest.id, reportData).subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage = err.error?.message || 'Failed to record test results. Please try again.';
        this.isSubmitting = false;
      },
    });
  }
