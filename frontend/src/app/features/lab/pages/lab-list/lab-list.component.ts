...
export class LabListComponent implements OnInit {
  ...
  initForm(): void {
    this.labForm = this.fb.group({
      ...
      testCode: ['', [Validators.required, Validators.maxLength(50), Validators.pattern(CODE_PATTERN)]],
  ...
  onSubmit(): void {
    ...
    if (this.showEditForm && this.editingTest) {
      this.labService.update(this.editingTest.id, formValue).subscribe({
        next: () => {
          this.statusModalService.showSuccess('Lab Test Updated', 'The lab request has been updated.');
          this.isSubmitting = false;
          this.closeForm();
          this.loadTests();
        },
        error: (err: HttpErrorResponse) => {
          this.statusModalService.showError('Update Failed', err.error?.message || 'Update failed.');
          this.isSubmitting = false;
        },
      });
    } else {
      this.labService.create(formValue).subscribe({
        next: () => {
          this.statusModalService.showSuccess('Lab Test Requested', 'A new lab request has been created.');
          this.isSubmitting = false;
          this.closeForm();
          this.loadTests();
        },
  ...
  onUpdateStatus(test: LabTest, status: TestStatus): void {
    this.labService.updateStatus(test.id, status).subscribe({
      next: () => {
        this.statusModalService.showSuccess('Lab Status Updated', `Test status changed to ${status}.`);
        this.loadTests();
      },
      error: () => this.statusModalService.showError('Status Update Failed', 'Lab test status could not be updated.'),
    });
  }
  ...
  onDelete(id: string): void {
    ...
    this.labService.delete(id).subscribe({
      next: () => {
        this.statusModalService.showSuccess('Lab Test Deleted', 'The lab request has been removed.');
        this.loadTests();
      },
      error: (err) => {
  ...
  canRecordResults(): boolean {
    return this.userRole === 'LABORATORY_STAFF' || this.userRole === 'ADMIN';
  }
